...
def load_and_clean_data(file_path='components/data/cleaned_dataset.csv'):
    # Load data
...
def train_xgb_default(X_resampled, y_resampled, X_test):
...
def train_lgb_default(X_resampled, y_resampled, X_test):
...
def train_rf_default(X_resampled, y_resampled, X_test):
...
def train_ensembeled_default(rf_default, xgb_default, X_resampled, y_resampled, X_test):
    rf_xgb_default = VotingClassifier(
        estimators=[('rf', rf_default), ('xgb', xgb_default)],
        voting='soft'
    )
    rf_xgb_default.fit(X_resampled, y_resampled)
    y_pred_ensemble_default = rf_xgb_default.predict(X_test)
...
def compute_sample_weights(y_resampled):
    """
    ...
    sample_weights = np.array([weight_dict[label] for label in y_resampled])  # Create sample weights for each training instance
    return sample_weights, weight_dict




def train_xgb_tuned(X_resampled, y_resampled, X_test, sample_weights):
    """
    ...
    xgb_tuned.fit(X_resampled, y_resampled, sample_weight=sample_weights)
...
def train_lgb_tuned(X_resampled, y_resampled, X_test, sample_weights):
    """
    ...
    lgb_tuned.fit(X_resampled, y_resampled, sample_weight=sample_weights)
...
def train_rf_tuned(X_resampled, y_resampled, X_test, sample_weights):
    """
    ...
    rf_tuned.fit(X_resampled, y_resampled, sample_weight=sample_weights)
...
#--------------------------------
def train_ensemble_tuned(rf_tuned, xgb_tuned, X_resampled, y_resampled, X_test):
    """
    ...
    ensemble_tuned.fit(X_resampled, y_resampled)
...
def kpi_reports(model, X_test, y_test):
    """
...
def model_pipeline():
    ...
    X_resampled, X_test, y_resampled, y_test = load_and_clean_data()

    xgb_default, y_pred_xgb_default = train_xgb_default(X_resampled, y_resampled, X_test)
    lgb_default, y_pred_lgb_default = train_lgb_default(X_resampled, y_resampled, X_test)
    rf_default, y_pred_rf_default = train_rf_default(X_resampled, y_resampled, X_test)
    rf_xgb_default, y_pred_ensemble_default = train_ensembeled_default(rf_default, xgb_default, X_resampled, y_resampled, X_test)


    sample_weights, weight_dict = compute_sample_weights(y_resampled)


    xgb_tuned, y_pred_xgb_tuned, best_params_xgb = train_xgb_tuned(
        X_resampled, y_resampled, X_test, sample_weights
    )

    lgb_tuned, y_pred_lgb_tuned, best_params_lgb = train_lgb_tuned(
        X_resampled, y_resampled, X_test, sample_weights
    )

    rf_tuned, y_pred_rf_tuned, best_params_rf = train_rf_tuned(
        X_resampled, y_resampled, X_test, sample_weights
    )

    ensemble_tuned, y_pred_ensemble_tuned, best_params_ensemble = train_ensemble_tuned(
        rf_tuned, xgb_tuned, X_resampled, y_resampled, X_test
    )


    auc_xgb_default = compute_auc(xgb_default, X_test, y_test)
    auc_lgb_default = compute_auc(lgb_default, X_test, y_test)
    auc_rf_default = compute_auc(rf_default, X_test, y_test)
    auc_ensemble_default = compute_auc(rf_xgb_default, X_test, y_test)
    ...
    fpr_xgb_default, tpr_xgb_default = compute_roc_curve(xgb_default, X_test, y_test)
    fpr_lgb_default, tpr_lgb_default = compute_roc_curve(lgb_default, X_test, y_test)
    fpr_rf_default, tpr_rf_default = compute_roc_curve(rf_default, X_test, y_test)
    fpr_ensemble_default, tpr_ensemble_default = compute_roc_curve(rf_xgb_default, X_test, y_test)
    ...
    roc_auc_results["XGB Default"] = {
        "auc": compute_auc(xgb_default, X_test, y_test),
        "fpr": compute_roc_curve(xgb_default, X_test, y_test)[0],
        "tpr": compute_roc_curve(xgb_default, X_test, y_test)[1]
    }

    roc_auc_results["LGB Default"] = {
        "auc": compute_auc(lgb_default, X_test, y_test),
        "fpr": compute_roc_curve(lgb_default, X_test, y_test)[0],
        "tpr": compute_roc_curve(lgb_default, X_test, y_test)[1]
    }

    roc_auc_results["RF Default"] = {
        "auc": compute_auc(rf_default, X_test, y_test),
        "fpr": compute_roc_curve(rf_default, X_test, y_test)[0],
        "tpr": compute_roc_curve(rf_default, X_test, y_test)[1]
    }

    roc_auc_results["Ensemble Default"] = {
        "auc": compute_auc(rf_xgb_default, X_test, y_test),
        "fpr": compute_roc_curve(rf_xgb_default, X_test, y_test)[0],
        "tpr": compute_roc_curve(rf_xgb_default, X_test, y_test)[1]
    }
    ...
    kpis_dict["XGB Default"] = kpi_reports(xgb_default, X_test, y_test)
    kpis_dict["LGB Default"] = kpi_reports(lgb_default, X_test, y_test)
    kpis_dict["RF Default"] = kpi_reports(rf_default, X_test, y_test)
    kpis_dict["Ensemble Default"] = kpi_reports(rf_xgb_default, X_test, y_test)
